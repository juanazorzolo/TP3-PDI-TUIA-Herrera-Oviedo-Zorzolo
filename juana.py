...
def eliminar_carpetas_contenido(carpeta1):
    carpetas = [carpeta1]
...
eliminar_carpetas_contenido('capturas-dados')
os.makedirs("capturas-dados", exist_ok=True)
...
# Lista de archivos en la carpeta "videos-dados"
carpeta_videos = "videos-dados"
archivos_videos = [f for f in os.listdir(carpeta_videos) if f.endswith(('.mp4', '.avi', '.mov'))]

# Procesar cada video
for archivo_video in archivos_videos:
    ruta_video = os.path.join(carpeta_videos, archivo_video)
        ...
        if area:
            ...
            if frame_anterior is not None:
                ...
                if nivel_movimiento < 1000 and not frame_guardado:  # Ajustar el umbral si es necesario
                    ...
                    nombre_frame = f"{os.path.splitext(archivo_video)[0]}_dado_quieto.png"
                    ruta_frame = os.path.join("capturas-dados", nombre_frame)
